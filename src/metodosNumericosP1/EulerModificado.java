...
public class EulerModificado {
    ...
    public void metodoEulerModificado (JTextArea jTextArea, String funcion, double x0, double yx0, double h, double xf) {
        JEP jep = new JEP();
        ...
        for (double x = x0; x <= xf-h+(h/50);) {
            ...
            k1 = jep.getValue();
            double k2 = jep.addVariable("x", x + h);
            k2 = jep.addVariable("y", y + h * k1);
            jep.parseExpression(funcion);
            k2 = jep.getValue();
            y += (h / 2)*(k1 + k2);
            x += h;
